...
// Dynamic input with non-block-aligned feature: verify onednn is rejected and output has no NaN.
// Feature is static (f=24, 24%16!=0), batch is dynamic → node.is_dynamic() == true.
// Output feature f=48 is aligned (48%16==0, passes existing output check), so the fix's
// input feature alignment check is the only guard rejecting onednn for this case.
TEST(concat_gpu_onednn, dynamic_non_block_aligned_feature) {
    ...
    // Dynamic batch, static feature (non-block-aligned f=24)
    ...
    // Prepare data in b_fs_yx_fsv16 physical layout with NaN in padding slots
    ...
    // Inject NaN into padding slots (simulates memory pool reuse without zero-fill)
    ...
    // Verify: onednn should NOT be selected for dynamic non-block-aligned concat
    auto concat_inst = network->get_primitive("concat");
    auto impl = concat_inst->get_impl();
    ASSERT_TRUE(impl != nullptr);
    ASSERT_TRUE(impl->m_manager != nullptr);
    ...
    // Verify: output should contain no NaN
