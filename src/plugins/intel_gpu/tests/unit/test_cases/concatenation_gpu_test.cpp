...
TEST(concat_gpu_onednn, impl_selection_non_block_aligned_feature) {
    // For static shapes, onednn concat correctly handles non-block-aligned features via
    // logical memory descriptors. Only dynamic shapes reject onednn for non-aligned features
    // (to avoid NaN issues with memory reuse in Loop body iterations).
    auto& engine = get_test_engine();
    if (!engine.get_device_info().supports_immad)
        return;
    ...
    ASSERT_TRUE(impl != nullptr);
    ASSERT_TRUE(impl->m_manager != nullptr);
    // Static non-block-aligned feature: onednn is accepted (handles correctly via logical descriptors)
