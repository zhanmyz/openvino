...
//
// Overflow occurs when: K × max|W| > FP16_MAX (IEEE 754 binary16 max = 65504)
// Equivalently: max|W| > FP16_MAX / K
//
// FP16_MAX is obtained via std::numeric_limits<ov::float16>::max() rather than a
// magic constant, consistent with OpenVINO's type-system conventions.
//   K = reduction dimension of the weight constant (from shape)

// Largest finite value of IEEE 754 binary16 (half-precision).
static const float fp16_max_finite = static_cast<float>(std::numeric_limits<ov::float16>::max());

// Returns true when max|W| > FP16_MAX / K (Cauchy-Schwarz overflow criterion).
// NOTE: assumes unit-RMS activations entering the MatMul; FFN gated intermediates
// can have much larger norms and may overflow even when this function returns false.
bool weights_exceed_fp16_overflow_criterion(const std::shared_ptr<ov::Node>& matmul_node) {
    ...
    for (int depth = 0; depth < 10 && node; ++depth) {
        if (auto constant = ov::as_type_ptr<ov::op::v0::Constant>(node)) {
            ...
            const float overflow_bound = fp16_max_finite / k_f;
...
DisableFP16CompForRMSWithFullPrecisionWeights::DisableFP16CompForRMSWithFullPrecisionWeights() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        // Mark RMS only if the upstream MatMul weights exceed the FP16 overflow criterion
        if (!weights_exceed_fp16_overflow_criterion(upstream_matmul)) {
            return false;
        }
