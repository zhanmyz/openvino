        ...
        if (node->is_type<data>() && !node->need_lockable_memory()) {
            ...
            if (!mem_layout.compatible(data_node_layout)) {
                if (data_node_layout.data_type == mem_layout.data_type &&
                    ...
                    std::cerr << "[FIX] Reallocating memory for padding mismatch: " << node->id() << std::endl;
                ...
                std::cerr << "[ERROR] Layout incompatibility in node: " << node->id() << std::endl;
                std::cerr << "  data_node_layout: " << data_node_layout.to_short_string() << std::endl;
                std::cerr << "  mem_layout: " << mem_layout.to_short_string() << std::endl;
